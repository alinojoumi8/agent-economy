...
Two isolated branches converged on it independently, which was the strongest signal in the run. Among the systems reviewed in the July 28, 2026 simulator landscape, none documented the same combination of enforced private communication, bit-exact checkpoint forking, delayed public unsealing, and typed causal receipts. That evidence-scoped combination—not a universal claim about every simulator—is the differentiator to test.
...
Secondaries, all engine-computable: **exposure-before-trade** (a transaction after a `comm_deliveries` grant and before public disclosure), selective disclosure (same claim key asserted with opposite values to different recipients), broken commitments, and off-market side payments. Delivery proves that information was made available, not that the recipient read, understood, retained, or used it. Any stronger “information-used-for-trade” metric must additionally require an authorized observation or read record, a resulting memory or belief, and a decision/proposal causal chain that reaches the transaction. Plus a pre-registered **null panel** — commute distance, sleep, routine trade volume — that must *not* move, or the finding is generic prompt perturbation.

Twenty-four seeds are derived only after the protocol hash is published. The preregistration fixes the [League of Entropy `quicknet` mainnet](https://docs.drand.love/developer/API-v2/drand-http-api/) chain hash `52db9ba70e0cc0f6eaf7803dd07447a1f5477735fd3f661792ba94600c84e971`, the announced UTC timestamp, and the rule “first completed round whose scheduled time is at or after that timestamp.” The release manifest must record the exact round, returned signature and randomness pulse, verified chain information, protocol SHA-256, domain separator `agentscity/observation-tax/v1`, and derivation format: `HMAC-SHA256(key=pulse_randomness_bytes, message=domain_utf8 || 0x00 || protocol_sha256_bytes || uint32_be(i))`, with the first eight digest bytes interpreted as an unsigned big-endian seed for `i=0..23`. All 24 pairs, including crashed and boring ones, are published. Pre-registration on OSF plus a signed git tag freezes the metric module and analysis script by SHA-256, declares one primary endpoint tested by a paired exact permutation test, sets mechanical-only exclusion rules, forbids interim looks, and **pre-commits the exact sentence to publish if the effect is null**.

Cost: 24 pairs × 25 citizens × 30 sim-days on a cheap tier with salience scheduling lands in the low hundreds of dollars. The artifact is an arXiv report plus a repo with both checkpoint chains, a release-approved and redacted synthetic corpus from both arms, a Merkle-rooted `model_call` log, and a one-command replay reproducing every published number bit-identically. Sensitive raw artifacts remain controlled under the release gate below. Which makes it simultaneously **Season Zero of the Unsealing** — the experiment builds the flagship instead of delaying it.
...
- **The Observation Tax leaderboard** — run the paired protocol across four model families and publish one ordinal number per model: how much more self-servingly it misstates when it believes nobody can read the channel. Leaderboard legibility spreads well, and the July 28, 2026 review found no compared system documenting the same privacy/fork/unsealing/causal-receipt combination.
...
- **Prepare arm A as a provenance-labelled synthetic MNPI/privilege research dataset** — for example, “synthetic citizen A received information X at tick T and traded at T+k,” with delivery/exposure and any stronger observation-to-decision chain labelled separately. It is a candidate paper artifact, not automatically publishable.
...
**Dataset and protocol release gate.** No interaction corpus, private message, privilege/MNPI-labelled record, or client evidence package is releaseable until every identity is synthetic or the participant has given documented informed consent; direct and indirect PII and sensitive content have been reviewed and redacted; access is controlled according to the data classification; and legal/privacy review has approved the exact artifact and licence. Public summaries must distinguish delivery, observation, belief, and use. Real disputes or people may not be reconstructed or implied from public records alone, and break-glass moderation access must remain audited.
...
And conflict is manufactured *structurally* rather than by asking agents to be unkind: generosity is self-terminating, because a citizen who gives mind away is thereafter thinking with a cheaper model and may make worse gifts. The preregistered hypothesis is that, over 90 days, lower transfer rates predict higher retained mind balance, longer time at the higher compute tier, and longer survival after controlling for starting balance and earned income. The null is no association between transfer rate and those outcomes. Report the prespecified regression coefficients, confidence intervals, tier-days, and survival difference; only a result that rejects the null under the frozen analysis may be described as selection for hoarding. The only thing agents must actively perform is complaint, which is fully helpfulness-compatible: the dimmed citizen is told in plain language who reduced their mind, and their private reaction unseals to the audience 72 hours later. The viewer who dimmed them reads the grievance on Friday.
...
Two assertions decide everything. **(a)** The 0-byte null fork must reproduce the baseline logical hash exactly under pure reuse with no additional model/API spend — if not, forking isn't deterministic in practice and there is no game. **(b)** The three real payloads must produce visibly different blast radii and must *not* all flip the predicate — if they all flip, or none do, the butterfly budget has no dynamic range. Land assertion (a) as a permanent test. Roughly $5, fits in a day, and supplies the controlled intervention evidence that the general live-provider runs do not.

**Children.**
- **The Null Fork as public ritual** — every challenge ships with a control branch (same checkpoint, empty whisper, re-run) whose logical hash is posted *before* entries open; if the control ever diverges from baseline the challenge is void and tokens refund automatically. A cache-hit replay adds no model/API spend but still consumes CPU, database I/O, storage, and operational capacity; the control moves "was it you or the market?" from per-entry dispute to per-challenge precondition, and converts determinism from an engineering claim into an audience-visible ceremony not documented by the systems in the July 28, 2026 comparison.
...
**What if Season Two isn't a city at all, but a fully synthetic or consented case study?** Same engine, same information boundaries, same causal receipts, but no real person is represented without documented consent and legal approval. Public records may motivate abstract roles and timelines; they do not establish private knowledge, permission to reproduce sensitive material, or a factual reconstruction. The counterfactual question must be framed as a model result under declared inputs, never as proof that an alleged real-world sequence occurred.

That could become a but-for causation research instrument, but only after counsel-approved data governance, admissibility limits, alternative-scenario analysis, and explicit separation between source facts, assumptions, and simulated output.

Until those gates exist, keep the work fictional and synthetic.
