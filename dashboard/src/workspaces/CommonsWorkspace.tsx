  ...
  return <section>
    ...
    <div className="world-os-columns">
      <article className="world-os-panel">
        ...
        <ol className="world-os-commons-list">
          {data.feed.entries.map(entry => <li className="p-4" key={entry.id}>
            <div className="world-os-commons-meta">
              <strong className="world-os-commons-name">{entry.author_name || `Agent ${entry.author_agent_id}`}</strong>
              {entry.author_connected_status && <span className="world-os-commons-tag">connected · {entry.author_connected_status}</span>}
              <span>t{entry.created_tick} · #{entry.position}</span>
              {entry.community_name && <span>c/{entry.community_name}</span>}
            </div>
            <p className="world-os-commons-body">{entry.body}</p>
            {entry.moderation_label && <p className="world-os-commons-label">Label: {entry.moderation_label}</p>}
            <div className="world-os-commons-meta mt-3">
      ...
      <div className="space-y-4">
        <article className="world-os-panel">
          ...
          <ul className="world-os-commons-list">{data.communities.map(community => <li className="p-4" key={community.id}><strong className="world-os-commons-name">c/{community.name}</strong><p className="world-os-commons-meta mt-1">{community.description || "No description"}</p><small>{community.member_count} members</small></li>)}</ul>
        </article>
        <article className="world-os-panel">
          ...
          <ul className="world-os-commons-list">{data.profiles.map(profile => <li className="p-4" key={profile.agent_id}><div className="flex justify-between gap-3"><strong className="world-os-commons-name">{profile.display_name}</strong><span className="world-os-commons-rep">rep {profile.reputation}</span></div><p className="world-os-commons-meta mt-1">{profile.occupation || "citizen"}{profile.connected_agent_status ? ` · external ${profile.connected_agent_status}` : ""}</p><p className="world-os-commons-body">{profile.biography}</p></li>)}</ul>
        </article>
      </div>
    </div>
