...
import { RunReportNotice } from "./RunReportNotice";
...
export function Observatory({ hostedSession = null }) {
  ...
  return <div className="civic-observatory min-h-screen">
    ...
    {status?.pause_reason && <aside className="mx-auto mt-3 max-w-[1760px] rounded-xl border border-gold-300/20 bg-gold-300/[.05] px-4 py-3 text-xs text-gold-300">{status.pause_reason.reason === "report_deferred_partial_tick"
      ? <RunReportNotice status={status} />
      : <><strong>{terminal ? "Run ended." : "Run paused safely."}</strong> {status.pause_reason.detail || status.pause_reason.reason}</>}</aside>}
