...
>
> **Code baseline:** `codex/legal-political-economy-v2`
>
> **Database / maintained engine contract:** schema v11 / semantics 7
>
> **Release state:** the owner authorized merging this closure after a fresh
> exact-head CI matrix. Tagging and publication remain separate actions.
...
privacy/provenance checks found zero defects. The merge procedure requires the
exact pushed head to pass the dashboard job and complete Ubuntu/Windows Python
3.11/3.12 matrix before PR #15 is made ready and merged.

## Semantics-7 closure matrix

| Surface | Implemented contract | Repository evidence | Final evidence state |
|---|---|---|---|
| Versioning | Maintained profiles select semantics 7; markerless and stored semantics 1–6 retain historical behavior; explicit forks may opt in; schema remains v11. | [Base profile](../runs/base.yaml), [v2 profile](../runs/v2.yaml), replay compatibility tests | **Passed:** 86 initial focused, 93 final adversarial, and current 280-test full gates |
...
| Replay fixture | Physical LLM row IDs canonicalize through deterministic referenced-call content; missing/dangling/wrong logical references fail. Fixture v2 strips raw provider envelopes, uses `repo://` paths, and restores recorded dataset/calibration/scenario inputs instead of current manifests. | [Verifier](../world/replay_verify.py), [portable fixture test](../tests/test_recorded_replay_golden.py), [fixture](../tests/golden/fd0adc5dc1.sqlite.json.zlib.b64) | **Passed:** semantics-5 fixture artifact `af57eed5…952d7`, normalized replay `2efcabed…f9170`, and semantics-7 replays exact |
...
| Full Python / data | **Passed:** 280 tests in 165.73 seconds; compileall green; FRED/BLS three required targets each, four optional sources explicitly unpinned. |
...
| CI / PR | **Merge condition:** exact pushed head must pass dashboard plus Ubuntu/Windows Python 3.11/3.12 before ready/merge. |
...
These results authorize this code merge only. Repeat the provenance, license,
dependency, and secret audit against the release candidate before a public tag.

## Release constraints and deferred campaigns

- Merge PR #15 only after its exact-head CI matrix passes. Do not tag or publish
  as part of this closure merge.
