...
- Final maintained-suite verification on 2026-08-05 at candidate `1768f96`: all eight invocations of `pytest tests/ -q -p scripts.pytest_shard --ci-shard-index N --ci-shard-count 8` (N = 0..7) passed with 1,289 tests passed, seven environment-gated skips, and zero failures. The dashboard passed 104 unit tests, type checking, license and high-severity dependency gates, a byte-identical repeat production build, and all 31 Chromium end-to-end tests.
