...
- Numeric grounding now uses one pure Decimal-based token contract for money, percentages, signs, commas, malformed separators, and recursive bounded sources. Derived arithmetic is rejected even when its operands are supplied.
- The next-untouched activation boundary is persisted without changing caller profiles. After activation, model-authored reserved beliefs require a baseline and stay within the configured finite step; scripted/direct engine updates retain their prior path.
- Public decision rationales and condensed model memories retain exact supplied numerics or fall back to number-free text. The raw governed `llm_calls.response_json` remains unchanged for audit. Focused research, compatibility, memory-ranking, replay, and PRD verification passed 115 tests.
- News, report, API, and replay projections now reject model-authored numbers absent from their cited bounded facts after the activation boundary, emit `ungrounded_numeric_claim` telemetry, and retain raw governed provider records only in the existing audit store.
- Dashboard metric, delta, trust, cents, belief, replay, and partial-day displays share unit-aware formatters. Memories and raw model outputs carry authority warnings; projected numeric redactions carry a visible marker.
- Public-grounding verification on 2026-08-05: 47 focused Python tests, 67 dashboard tests, type checking, license and dependency audits, the Chromium privacy canary test, and two byte-identical production builds passed.
- Final maintained-suite verification on 2026-08-05 at source candidate `f47abae`: all eight invocations of `pytest tests/ -q -p scripts.pytest_shard --ci-shard-index N --ci-shard-count 8` (N = 0..7) passed with 1,290 tests passed, seven environment-gated skips, and zero failures. The dashboard passed 108 unit tests, type checking, license and high-severity dependency gates, a byte-identical repeat production build, and all 32 Chromium end-to-end tests.
