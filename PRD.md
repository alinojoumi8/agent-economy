...
| LLM backend | **Provider-agnostic hybrid.** The gateway maps each role to any {provider, model} in config. Defaults: citizens + conversations + memory on **MiniMax M3 Token Plan**; ~8 high-leverage seats (central banker, credit officers, editors/lead reporters, VC partner, Oracle) on **Kimi K2.6 through the Kimi API Platform**. Kimi Code membership remains an optional compatibility profile but cannot pin K2.6. Anthropic API is an optional alternative tier. **Claude/Codex/Grok CLI subscriptions are never used for the agent swarm** — rate limits and provider terms rule them out for bulk inference; Claude CLI (headless) is permitted for the Oracle and development only. | 100 agents thinking daily is cost-dominated by citizens; the M3/K2.6 split keeps strong-model calls concentrated in high-leverage roles. Per-role provider config also enables model-vs-model economy experiments. |
