...
| LLM | **Provider-agnostic gateway.** Adapters: OpenAI-compatible (covers Kimi/Moonshot and MiniMax — both expose OpenAI-style APIs), Anthropic, and a restricted CLI adapter (Oracle/dev only). Locked target: citizens/conversations/memory → `MiniMax-M3`; strong seats + Oracle → `kimi-k2.6` through `https://api.moonshot.ai/v1`. Routing is `role → {provider, model}` in `run.yaml`, not code. | The active `runs/production.yaml` profile temporarily routes every role to MiniMax M3. The locked mixed-provider target is preserved as the explicit `runs/production-k2.6.yaml` acceptance profile; Kimi Code remains a separate compatibility profile. |
