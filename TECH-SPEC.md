...
- V4 seeds 7331 and 7332 completed with exact companion replays and remain diagnostic evidence only. Seed 7333 exposed an advertised-government-ledger mapping defect: the governed catalog offered `gov`, while execution looked only for `owner_type='gov'` instead of the system-owned treasury account. The resulting `entity ledger accounts not found` execution failure was recorded as though it were an independently reproducible preflight rejection, making the scheduled forecast and therefore the run ineligible. The active v5 contract corrects the government mapping, shares its historical state-aware catalog/preflight between runtime and audit, and separates post-preflight execution failures. No v4 source, response, claim, initialized marker, checkpoint, replay, profile, commitment, manifest entry, or seed is reused in v5.
