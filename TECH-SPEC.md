...
- Optional operational storage policy compresses large model-call bodies losslessly in SQLite, preserving exact logical replay/hash values and all memory rows. It retains a bounded verified recovery set with pins, checks run/tenant/free-space admission, and supports verified self-contained archives and plain database exports. Hostinger streaming mode disables periodic full copies, keeps pause/stop recovery points and restores missing run caches through pinned Litestream/SFTP without an implicit stale-snapshot fallback. See [storage and recovery](docs/storage-and-recovery.md) and [Hostinger VPS](docs/hostinger-vps.md); neither changes recorded economic semantics.
