...
from tests.test_jev_contract import (
    evaluation,
    response as openrouter_response,
    transport,
)


def response():
    value = openrouter_response()
...
def configuration():
    return {
        ...
        "budget": {"cap_usd": 5, "oracle_reserve_usd": 0},
        "llm": {
            "default_route": {"provider": "scripted", "model": "scripted"},
            "pricing": {"jev-1.13.0": {"in": 0.042, "out": 0.0}},
            "provider_retries": 0,
            "providers": {
                "jev": {"kind": "typesafe_decisions", "api_key_env": "TEST_JEV_KEY"}
            },
            "decision_policy": {
                "version": "bounded-economic-choice-v1",
...
def request(**updates):
    return LLMRequest(
        **({"role": "citizen", "purpose": "decision", "tick": 1} | updates)
    )


def test_advisory_calls_use_explicit_reserve_without_changing_world_cadence(
    store, monkeypatch
):
    monkeypatch.setenv("TEST_JEV_KEY", "private-fixture-value")
    config = configuration()
    config["budget"].update(cap_usd=1, helper_reserve_usd=0.8)
    body = response()
    calls = []
    transport(
        monkeypatch, lambda req: calls.append(req) or httpx.Response(200, json=body)
    )
    gateway = Gateway(store, config)
    try:
        first = asyncio.run(
            gateway.evaluate(request(purpose="hermes_selection"), evaluation())
        )
        repeat = asyncio.run(
            gateway.evaluate(request(purpose="hermes_selection"), evaluation())
        )
        ...
        assert not gateway.governor.can_spend(0.8, "commons_selection")
    ...
    try:
        with pytest.raises(BudgetExceeded):
            asyncio.run(
                gateway.evaluate(request(purpose="commons_selection"), evaluation())
            )
    finally:
        gateway.close()


def test_openrouter_rejects_other_models_but_keeps_recorded_replay_and_direct_providers(
    store, monkeypatch
):
    ...
    try:
        with pytest.raises(ProviderConfigurationError, match="restricted to Jev"):
            asyncio.run(
                gateway._dispatch_completion(
                    "old_router",
                    gateway.adapters["old_router"],
                    "openai/gpt-4.1-mini",
                    [],
                )
            )
        ...
        gateway.close()
    assert (
        openrouter_route_error(
            {"kind": "openai_compat", "base_url": "https://api.deepseek.com"},
            "deepseek-flash",
        )
        is None
    )
...
def test_zero_helper_reserve_blocks_even_a_zero_cost_estimate(store, monkeypatch):
    monkeypatch.setenv("TEST_JEV_KEY", "private-fixture-value")
    transport(
        monkeypatch, lambda req: pytest.fail("no provider call may be dispatched")
    )
...
def test_replay_rollback_restores_advisory_allowance(tmp_path, monkeypatch):
    ...
    config = configuration()
    config["budget"]["helper_reserve_usd"] = 0.01
    ...
    try:
        ...
        assert gateway.governor.can_spend(0.01, "hermes_selection")
...
def test_gateway_prices_reported_cost_and_reuses_durable_call(store, monkeypatch):
    ...
    calls = []
    body = response()
    body["usage"]["cost"] = 0.00123  # Deliberately differs from a tariff estimate.
    transport(
        monkeypatch, lambda req: calls.append(req) or httpx.Response(200, json=body)
    )
    gateway = Gateway(store, configuration())
    ...
    assert first.cost_usd == second.cost_usd == 0.00123
    assert gateway.governor.total_spend() == 0.00123
    ...
    assert (
        "private-fixture-value" not in record["request_json"] + record["response_json"]
    )
...
def test_recorded_choice_does_not_need_another_budget_reservation(store, monkeypatch):
    ...
    assert (
        asyncio.run(gateway.evaluate(request(), evaluation())).call_id == first.call_id
    )
    assert len(calls) == 1


def test_prose_call_on_same_provider_does_not_set_typed_model_identity(
    store, monkeypatch
):
    monkeypatch.setenv("TEST_JEV_KEY", "fixture-secret")
    store.insert(
        ...
        role="preflight",
        purpose="preflight",
        cache_key="prose-smoke",
        request_json="{}",
        response_json='{"text":"{\\"ok\\":true}"}',
        in_tokens=1,
        out_tokens=1,
        cost_usd=0,
        cached=0,
...
def test_typed_budget_checks_request_size_before_dispatch(store, monkeypatch):
    monkeypatch.setenv("TEST_JEV_KEY", "private-fixture-value")
    config = configuration()
    config["budget"]["cap_usd"] = 0.0000001
    gateway = Gateway(store, config)
    with pytest.raises(BudgetExceeded):
        asyncio.run(gateway.evaluate(request(), evaluation()))
    assert gateway._live_dispatch_count == 0
...
def test_cli_typed_preflight_keeps_durable_receipts_and_allowance(
    ...
    def handler(req):
        ...
        return httpx.Response(
            status, json=body if status == 200 else {"error": "denied"}
        )
    ...
    with sqlite3.connect(evidence.with_suffix(".jev-budget.db")) as conn:
        state, reserved = conn.execute(
            "SELECT state,reserved_cost FROM reservations"
        ).fetchone()
